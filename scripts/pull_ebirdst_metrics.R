...
abd <- load_raster(species_name, product = "abundance", period = "full-year", resolution = "3km")
...
abd_pa_laea_df <- as.data.frame(abd_pa_laea, xy = TRUE)

abd_pa_laea_df |> 
  ggplot(aes(x, y, fill = full_year)) +
  geom_tile() +
  scale_fill_viridis_c()
